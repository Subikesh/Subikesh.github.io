...
- `js/script.js` drives:
  ...
  - **Sync contract**: the section ids `hero/about/work/projects/contact` are referenced by the header nav links in `index.html`, and by `SECTION_IDS`/`initFab` in `script.js`. Renaming a section requires updating all of them.
